...
(defn render-banner []
  (div
   ...
   (div
    {}
    (a
     {:attrs {:href "https://github.com/Cirru/respo-cirru-editor/wiki/Keyboard-Shortcuts",
              :inner-text "Keyboard Shortcuts",
              :target "_blank"},
      :style style-link})
    (comp-space "80px" nil)
    (a
     {:attrs {:href "https://www.youtube.com/playlist?list=PLyvBXLgHYHy1AIK6i5uw3_H5BIUP4CQx6",
              :inner-text "Videos",
              :target "_blank"},
      :style style-link}))))
...
(defn render-project [project address]
  (a {:style style-project, :attrs {:target "_blank", :href address, :inner-text project}}))

(defn on-update [mutate!] (fn [snapshot dispatch!] (mutate! snapshot) (focus!)))
...
(defn init-state [] {:tree demo-tree, :focus [], :clipboard []})
  ...
  (div
   ...
   (div
    ...
    (comp-text
     ...
     nil)
    (comp-space "8px" nil)
    (div {:style ui/button, :event {:click (on-show tree)}} (comp-text "Show me data" nil)))
   (comp-space nil "60px")
   (div
    {:style (merge typeset/title style-section-title)}
   ...
   (div
    {:style (merge typeset/content style-content)}
    (comp-text "I'm exploring new ways of programming with Cirru, such as " nil)
...
(defn render []
  (fn [state mutate!]
    (div
     ...
     (div {:style (merge ui/row style-source)} (render-code-intro (:tree state)))
     (render-links))))
